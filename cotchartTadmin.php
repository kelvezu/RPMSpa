...
if (isset($_POST['view'])) :
  if (empty($_POST['sy_id']) || empty($_POST['school_id'])) :
    ...
    $cot_array = [];
    $qry = $conn->query("SELECT * FROM cot_t_indicator_ave_tbl  WHERE sy = $sy  ") or die($conn->error);
      ...
      function drawVisualization() {
        ...
        var data = google.visualization.arrayToDataTable([
          ...
          <?php
            ...
            FROM cot_t_rating_a_tbl WHERE indicator_id = " . $indi['indicator_id'] . " AND sy = '$sy' group by obs_period");
                while ($rowQry = $cot2Qry->fetch_assoc()) :
                 ...
                 " . rawRate(fetchIndicatorAVGAdmintGeneral($conn, $indi['indicator_id'], $sy), $position) . ",],";
                endwhile;
              endforeach;
              ?>
        ]);
  ...
  (SELECT indicator_id,AVG(average) AS T_average,
    ...
    CASE WHEN sy = ('" . $_SESSION['active_sy_id'] . "')-1 THEN AVG(average) END AS sy2,
    CASE WHEN sy = '" . $_SESSION['active_sy_id'] . "' THEN AVG(average) END AS sy
    FROM cot_t_indicator_ave_tbl WHERE school = '$school_id'  GROUP BY indicator_id,average,sy) a
    GROUP BY a.indicator_id") or die($conn->error);
              while ($cotQry = $qry->fetch_assoc()) :
                echo "['" . $cotQry['indicator_id'] . "', 
          " . rawRate(($cotQry['sy3']), $position) . ",  
      ...
      function drawVisualization() {
        ...
        var data = google.visualization.arrayToDataTable([
          ...
          <?php
            ...
            CASE WHEN obs_period = 3 THEN rating END AS Period3, 
            CASE WHEN obs_period = 4 THEN rating END AS Period4 
            FROM cot_t_rating_a_tbl WHERE indicator_id = " . $indi['indicator_id'] . " AND sy = '$sy_id' AND school_id = '$school_id' group by obs_period");
                while ($rowQry = $cot2Qry->fetch_assoc()) :
                  echo "[" . $indi['indicator_id'] . ",
                 ...
                 " . rawRate(fetchIndicatorAVGAdmintGeneral($conn, $indi['indicator_id'], $sy_id), $position) . ",],";
                endwhile;
              endforeach;
              ?>
        ]);
