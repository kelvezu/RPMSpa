...
$cot_array = [];
$qry = $conn->query("SELECT * FROM cot_t_indicator_ave_tbl  WHERE sy = $sy_id AND school = $school_id  ") or die ($conn->error);
         ...
         foreach($tIndi as $indi):
            ...
            while ($rowQry = $cot2Qry->fetch_assoc()):
                echo "[".$indi['indicator_id'].",
                 ...
                 ".intval(fetchIndicatorAVGAdmint($conn, $indi['indicator_id'], $sy_id, $school_id)).",],";
            endwhile;
        endforeach;
         ?>
        ]);
