...
<?php
		$connect = new PDO('mysql:host=localhost;dbname=rpms', 'root', '');
		$query = "SELECT CONCAT(a.kra_id,'.',a.tobj_id,' ',a.tobj_name) 
				  AS OBJECTIVES, 
				CASE WHEN b.lvlcap = 1 then count(b.lvlcap) END AS L_LOW,
				CASE WHEN b.lvlcap = 2 then count(b.lvlcap) END AS L_MODERATE,
				CASE WHEN b.lvlcap = 3 then count(b.lvlcap) END AS L_HIGH,
				CASE WHEN b.lvlcap = 4 then count(b.lvlcap) END AS L_VERY_HIGH,
				
				CASE WHEN b.priodev = 1 then count(b.priodev) END AS P_LOW,
				CASE WHEN b.priodev = 2 then count(b.priodev) END AS P_MODERATE,
				CASE WHEN b.priodev = 3 then count(b.priodev) END AS P_HIGH,
				CASE WHEN b.priodev = 4 then count(b.priodev) END AS P_VERY_HIGH
				from tobj_tbl a LEFT JOIN esat2_objectivest_tbl b ON a.tobj_id = b.tobj_id
				
				group by a.kra_id,a.tobj_id,a.tobj_name;
				";
		$statement = $connect->prepare($query);
		$statement->execute();
		$result = $statement->fetchAll();
...
<?php
		$connect = new PDO('mysql:host=localhost;dbname=rpms', 'root', '');
		$query = "SELECT CONCAT(a.kra_id,'.',a.mtobj_id,' ',a.mtobj_name) 
				  AS OBJECTIVES, 
				CASE WHEN b.lvlcap = 1 then count(b.lvlcap) END AS L_LOW,
				CASE WHEN b.lvlcap = 2 then count(b.lvlcap) END AS L_MODERATE,
				CASE WHEN b.lvlcap = 3 then count(b.lvlcap) END AS L_HIGH,
				...
				group by a.kra_id,a.mtobj_id,a.mtobj_name;
				";
		$statement = $connect->prepare($query);
		$statement->execute();
		$result = $statement->fetchAll();
