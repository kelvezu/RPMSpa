...
<form  action="includes/processESATsurvey.php" method="POST"> <div class="text-center bg-primary font-weight-bolder">
                  <strong> <h3> Self Assessment Tool Form / Part I / Demographic Profile </h3></strong>
                  </div>
...
<div class="container">
    <div class="breadcome-list shadow-reset">
          <div class="bg-info"><strong>4. Position</strong></div>
                      ...
                      <?php  
                          $positionresult = $conn->query('SELECT * FROM position_tbl WHERE position_id != 1 AND position_id != 2')  or die($conn->error);
                            while($positionrow = $positionresult->fetch_assoc()):
                                $position_id= $positionrow['position_id'];
                                  $position_name = $positionrow['position_name'];
                      ?>
                            <option value="<?php echo $position_id ?>"><?php echo $position_name; ?>
                            <?php endwhile ?> 
                          </option>
                      </select>
                  </div>
