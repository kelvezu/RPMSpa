...
function cbcChart() {
    let data = google.visualization.arrayToDataTable([
       ['CBC Name','CBC Score'],
        ...
        $qry = mysqli_query($conn,"SELECT a.cbc_name,sum(b.cbc_score) AS cbc_score FROM core_behavioral_tbl a 
        INNER JOIN esat3_core_behavioralt_tbl b  on a.cbc_id = b.cbc_id WHERE b.user_id = '$user_id' AND sy = '$sy' group by a.cbc_name order by a.cbc_id") or die ($conn->error.$qry);
...
let options = {
        title : 'Core Behavioral Competencies',
            vAxis: {title: 'Rating', maxValue: 10},
            hAxis: {title: 'Core Behavioral', maxValue: 13, minValue: 1 },
            seriesType: 'bars',
            bar: { groupWidth: 30 },
            series: {5: {type: 'line'}}        };
...
function LevelofPriority() {
    let data = google.visualization.arrayToDataTable([
       ['Objective','Level of Capability','Level of Priority'],
        ...
        $qry = mysqli_query($conn,"SELECT CONCAT(a.kra_id,'.',a.tobj_id) as OBJECTIVES, lvlcap, priodev 
        ...
        group by a.tobj_id,b.tobj_name") or die ($conn->error.$qry);
...
function cbcChart() {
    let data = google.visualization.arrayToDataTable([
       ['CBC Name','CBC Score'],
        ...
        $qry = mysqli_query($conn,"SELECT a.cbc_name,sum(b.cbc_score) AS cbc_score FROM core_behavioral_tbl a 
        INNER JOIN esat3_core_behavioralt_tbl b  on a.cbc_id = b.cbc_id WHERE b.user_id = '$user' AND sy = '$sy_id' group by a.cbc_name order by a.cbc_id") or die ($conn->error.$qry);
