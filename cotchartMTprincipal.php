...
if (isset($_POST['view'])) :
  if (empty($_POST['sy_id']) || empty($_POST['teacher_id'])) :
    ...
    $cot_array = [];
    $qry = $conn->query("SELECT * FROM cot_mt_indicator_ave_tbl  WHERE sy = $sy AND school = '$school_id'  ") or die($conn->error);
    ...
    <script type="text/javascript">
      ...
      function chartCoT() {
        ...
        (SELECT indicator_id,AVG(average) AS T_average,
          ...
          FROM cot_mt_indicator_ave_tbl WHERE sy = $sy AND school = '$school_id'  GROUP BY indicator_id,average,sy) a
      ...
      function drawVisualization() {
        ...
        var data = google.visualization.arrayToDataTable([
          ...
          <?php
            ...
            FROM cot_mt_rating_a_tbl WHERE indicator_id = " . $indi['mtindicator_id'] . " AND sy = $sy AND school_id = '$school_id' group by obs_period");
                while ($rowQry = $cot2Qry->fetch_assoc()) :
                 ...
                 " . rawRate(fetchIndicatorAVGAdminMt($conn, $indi['mtindicator_id'], $sy, $school_id), $position) . ",],";
                endwhile;
              endforeach;
              ?>
        ]);
  ...
  (SELECT indicator_id,AVG(average) AS T_average,
    ...
    CASE WHEN sy = ('" . $_SESSION['active_sy_id'] . "')-1 THEN AVG(average) END AS sy2,
    CASE WHEN sy = '" . $_SESSION['active_sy_id'] . "' THEN AVG(average) END AS sy
    FROM cot_mt_indicator_ave_tbl WHERE sy = '$sy_id' AND `user_id` = '$teacher_id'   GROUP BY indicator_id,average,sy) a
    GROUP BY a.indicator_id") or die($conn->error);
              while ($cotQry = $qry->fetch_assoc()) :
                echo "['" . $cotQry['indicator_id'] . "', 
          " . rawRate(($cotQry['sy3']), $position) . ",  
      ...
      function drawVisualization() {
        ...
        var data = google.visualization.arrayToDataTable([
          ...
          <?php
            ...
            CASE WHEN obs_period = 3 THEN rating END AS Period3, 
            CASE WHEN obs_period = 4 THEN rating END AS Period4 
            FROM cot_mt_rating_a_tbl WHERE indicator_id = " . $indi['mtindicator_id'] . " AND  sy = '$sy_id' AND `user_id` = '$teacher_id' group by obs_period");
                while ($rowQry = $cot2Qry->fetch_assoc()) :
                  echo "[" . $indi['mtindicator_id'] . ",
                 ...
                 " . rawRate(fetchIndicatorAVGAdminMt($conn, $indi['mtindicator_id'], $sy_id, $school_id), $position) . ",],";
                endwhile;
              endforeach;
              ?>
        ]);
