...
if(isset($_POST['updateSUB'])){
    ...
    $qrySJ = mysqli_query($conn,"UPDATE subject_tbl SET subject_id = '$subject_id', subject_name = '$subject_name' WHERE subject_id = '$subject_id' ");
    if($qrySJ){
...
if(isset($_GET['deleteSJ'])){
    ...
    $conn->query("DELETE FROM subject_tbl WHERE subject_id=$subject_id") or die($conn->error);
...
if(isset($_GET['deleteAGE'])){
    ...
    $conn->query("DELETE FROM age_tbl WHERE age_id=$age_id") or die($conn->error);
    $_SESSION['message'] = 'Age has been deleted!';
    $_SESSION['msg_type'] = 'danger';
    header("location:../ESAT.php");
