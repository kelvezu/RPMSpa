        ...
        if (!mysqli_stmt_prepare($stmt, $sql)) {
            ...
            if ($row = mysqli_fetch_assoc($result)) {
                ...
                // check email if verified
                    $chkEmail = "SELECT * FROM account_tbl WHERE (email = '$userEmail' OR username = '$userEmail' ) AND email_status = 'Verified'";
                    $mailResults = mysqli_query($conn,$chkEmail) or die($conn->error);
                    $count_mail_results = mysqli_num_rows($mailResults);
                    if($count_mail_results > 0 ){
                        session_start();
                        ...
                        header("location:../loginpage.php?error=emailnotverified");
                        exit();
