...
if (isset($_POST['submit'])) {
    ...
    if (empty($prc_id) || empty($surname) || empty($firstname) ||  empty($email) || empty($contact)) {
        header("Location:../signup2.php?error=emptyfields&prc_id=" . $prc_id . "&surname=" . $surname . "&firstname=" . $firstname . "&middlename=" . $middlename . "&email=" . $email . "&contact=" . $contact);
        exit();
    }

    //CHECK IF THE PRCID LENGTH IS LESS THAN 11
    elseif (!is_numeric($prc_id) && !strlen($prc_id) >= 5) {
        header("Location:../signup2.php?error=shortprc_id&prc_id=" . $prc_id . "&surname=" . $surname . "&firstname=" . $firstname . "&middlename=" . $middlename . "&contact=" . $contact);
        exit();
    }
    ...
    elseif (!preg_match("/^[a-zA-Z ]*$/", $surname) && !preg_match("/^[a-zA-Z ]*$/", $firstname) && !preg_match("/^[a-zA-Z]*$/", $middlename)) {
        header("Location:../signup2.php?error=nospeccharnames&prc_id=" . $prc_id . "&email=" . $email . "&contact=" . $contact);
        exit();
    }
    //CHECK IF SURNAME AND FIRSTNAME CONTAINS SPECIAL CHARS
    elseif (!preg_match("/^[a-zA-Z ]*$/", $surname) && !preg_match("/^[a-zA-Z ]*$/", $firstname)) {
        header("Location:../signup2.php?error=nospeccharsnamefname&prc_id=" . $prc_id . "&middlename=" . $middlename . "&email=" . $email . "&contact=" . $contact);
        exit();
    }
    //CHECK IF FIRSTNAME AND MIDDLENAME CONTAINS SPECIAL CHARS
    elseif (!preg_match("/^[a-zA-Z ]*$/", $firstname) && !preg_match("/^[a-zA-Z ]*$/", $middlename)) {
        header("Location:../signup2.php?error=nospeccharfnamemname&prc_id=" . $prc_id . "&surname=" . $surname . "&email=" . $email . "&contact=" . $contact);
        exit();
    }
    //CHECK IF SURNAME AND MIDDLENAME CONTAINS SPECIAL CHARS
    elseif (!preg_match("/^[a-zA-Z ]*$/", $surname) && !preg_match("/^[a-zA-Z ]*$/", $middlename)) {
        header("Location:../signup2.php?error=nospeccharsnamemname&prc_id=" . $prc_id . "&firstname=" . $firstname . "&email=" . $email . "&contact=" . $contact);
        exit();
    }
    //CHECK IF EMAIL IS VALID
    elseif (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
        header("Location:../signup2.php?error=invalidemail&prc_id=" . $prc_id . "&surname=" . $surname . "&firstname=" . $firstname . "&middlename=" . $middlename . "&contact=" . $contact);
        exit();
    }
    //CHECK IF THE SURNAME CONTAINS SPECIAL CHARACTERS
    elseif (!preg_match("/^[a-zA-Z ]*$/", $surname)) {
        header("Location:../signup2.php?error=invalidsurname&prc_id=" . $prc_id . "&firstname=" . $firstname . "&middlename=" . $middlename . "&email=" . $email . "&contact=" . $contact);
        exit();
    }
    //CHECK IF THE FIRSTNAME CONTAINS SPECIAL CHARACTERS
    elseif (!preg_match("/^[a-zA-Z ]*$/", $firstname)) {
        header("Location:../signup2.php?error=invalidfirstname&prc_id=" . $prc_id . "&surname=" . $surname . "&middlename=" . $middlename . "&email=" . $email . "&contact=" . $contact);
        exit();
    }
    //CHECK IF THE MIDDLENAME CONTAINS SPECIAL CHARACTERS
    elseif (!preg_match("/^[a-zA-Z ]*$/", $middlename)) {
        header("Location:../signup2.php?error=invalidmiddlename&prc_id=" . $prc_id . "&surname=" . $surname . "&firstname=" . $firstname . "&email=" . $email . "&contact=" . $contact);
        exit();
    }

    //CHECK IF THE SURNAME LENGTH IS LESS THAN 1
    elseif (strlen($surname) <= 1) {
        header("Location:../signup2.php?error=shortsurname&prc_id=" . $prc_id . "&firstname=" . $firstname . "&middlename=" . $middlename . "&email=" . $email . "&contact=" . $contact);
        exit();
    }
    //CHECK IF THE FIRSTNAME LENGTH IS LESS THAN 1
    elseif (strlen($firstname) <= 1) {
        header("Location:../signup2.php?error=shortfirstname&prc_id=" . $prc_id . "&surname=" . $surname . "&middlename=" . $middlename . "&email=" . $email . "&contact=" . $contact);
        exit();
    }
    //CHECK IF THE MIDDLENAME LENGTH IS LESS THAN 1
    elseif (strlen($middlename) <= 1) {
        header("Location:../signup2.php?error=shortmiddlename&prc_id=" . $prc_id . "&surname=" . $surname . "&firstname=" . $firstname . "&email=" . $email . "&contact=" . $contact);
        exit();
    }
    //CHECK IF THE CONTACT IS NOT NUMERIC AND LENGTH IS NOT EQUAL TO 11 
    elseif (!is_numeric($contact) && !strlen($contact) == 11) {
        header("Location:../signup2.php?error=invalidcontact&prc_id=" . $prc_id . "&surname=" . $surname . "&firstname=" . $firstname . "&middlename=" . $middlename . "&email=" . $email);
        exit();
    }
    //CHECK IF THE CONTACT IS NOT NUMERIC
    elseif (!is_numeric($contact)) {
        header("Location:../signup2.php?error=contactnotnumeirc&prc_id=" . $prc_id . "&surname=" . $surname . "&firstname=" . $firstname . "&middlename=" . $middlename . "&email=" . $email);
        exit();
    }
    //CHECK IF THE CONTACT LENGTH IS EQUAL TO 11 
    elseif (strlen($contact) != 11) {
        header("Location:../signup2.php?error=contactshort&prc_id=" . $prc_id . "&surname=" . $surname . "&firstname=" . $firstname . "&middlename=" . $middlename . "&email=" . $email);
        exit();
    } else {
        $prcquery = "SELECT prc_id FROM account_tbl WHERE prc_id=?";
        $stmt = mysqli_stmt_init($conn);
        if (!mysqli_stmt_prepare($stmt, $prcquery)) {
            header("Location:../signup2.php?error=prciderror&surname=" . $surname . "&firstname=" . $firstname . "&middlename=" . $middlename . "&contact=" . $contact);
            exit();
        } else {
            ...
            if ($prccheck > 0) {
                header("Location../signup2.php?error=prctaken&surname=" . $surname . "&firstname=" . $firstname . "&middlename=" . $middlename . "&contact=" . $contact);
                exit();
            } else {
                ...
                if (!mysqli_stmt_prepare($stmt, $sql)) {
                    ...
                    if ($resultCheck > 0) {
                        header("Location:../signup2.php?error=emailtaken&prc_id=" . $prc_id . "&surname=" . $surname . "&firstname=" . $firstname . "&middlename=" . $middlename . "&contact=" . $contact);
                        exit();
                    } else {
                        ...
                        if ($position == "Principal") :
                            ...
                            if (mysqli_num_rows($chk_result) > 0) :
                                header('location:../signup2.php?error=oneprincipalonly');
                                exit();
                            endif;
                        endif;
                        ...
                        if (!empty($school)) {
                            ...
                            header('location:../signup2.php?error=emptyschool');
                            exit();
                        }
                    }
                }
