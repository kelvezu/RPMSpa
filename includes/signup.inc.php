        ...
        if (!mysqli_stmt_prepare($stmt, $sql)) {
            ...
            if ($resultCheck > 0) {
                ...
                if (!mysqli_stmt_prepare($stmt, $sql)) {
                    ...
                    if (!empty($school)) {
                        mysqli_stmt_bind_param($stmt, "ssssssssssi", nameFormat($surname), nameFormat($firstname), nameFormat($middlename), positionFormat($position), $email, $contact, ucwords($gender), $birthdate, $username, $hashedPwd, $school);
                    } else {
