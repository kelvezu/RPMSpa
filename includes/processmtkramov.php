...
if (isset($_POST['updatetOBJ'])) {
    ...
    $classroom_observable = $_POST['classroom_observable'];
    if ($classroom_observable == "No") {
        $map_indicator = "";
    } else {
        $map_indicator = $_POST['map_indicator'];
    }
    ...
    if ((ctype_space($mtobj_name))) {
        ...
        $update_res =  mysqli_query($conn, "UPDATE mtobj_tbl SET mtobj_id = '$mtobj_id', kra_id = '$kraid', mtobj_name = '$mtobj_name',classroom_observable = '$classroom_observable', indicator_id = '$map_indicator'   WHERE mtobj_id = '$mtobj_id' ") or die($conn->error);
...
if (isset($_POST['addobj'])) {
    ...
    $class_observable = $_POST['classroom_observable'];
    if ($classroom_observable == "No") {
        $map_indicator = "";
    } else {
        $map_indicator = $_POST['map_indicator'];
    ...
    if ($count_result > 0) {
        ...
        $query = "INSERT INTO mtobj_tbl(kra_id,mtobj_name,classroom_observable,indicator_id,date_created) VALUES('$select_kra','$addobjname','$class_observable','$map_indicator','" . datenow() . "')";
        $query_run = mysqli_query($conn, $query) or die($conn->$query);
        if ($query_run) {
