...
if (isset($_POST['submit_dp'])) :
    ...
    if ($check_dp_str && $check_dp_devneed && $check_dp_actionplan && $check_dp_str_behavioral && $check_dp_devneeed_behavioral && $check_dp_actionplan2) :
        header("location:../devplan_create_mt.php?notif=recordexist");
        exit();
    endif;
    ...
    if ($check_dp_str) :
        console_log("Record already exist in devplanmt_a1_strength_tbl");
    else :
    ...
    if ($check_dp_devneed) :
        console_log("Record already exist in devplanmt_a2_devneeds_tbl");
    else :
        for ($i = 0; $i < count($devneedobj); $i++) :
            $sql = "INSERT INTO `devplanmt_a2_devneeds_tbl`(`user_id`, `rater_id`, `sy`, `school`, `position`,  `devneeds_mtobj`, `status`,  `date_created`) VALUES ($user,$rater,$sy,$school,'" . $position . "',$devneedobj[$i],'Active','" . dateNow() . "')";
            $insert_sql_devneed_obj = mysqli_query($conn, $sql) or die($conn->error . $sql . 'b');
        endfor;
    endif;

    if ($check_dp_actionplan) :
        console_log("Record already exist in devplanmt_a3_actionplan_tbl");
    else :
        $sql = "INSERT INTO `devplanmt_a3_actionplan_tbl`( `user_id`, `rater_id`, `sy`, `school`, `position`, `a_learning_objectives`, `a_intervention`, `a_timeline`, `a_resources_needed`, `status`, `date_created`) VALUES ($user,$rater,$sy,$school,'$position','$a_learn_obj','$a_intervention','$a_timeline','$a_resource_needed','Active','" . dateNow() . "')";
        $insert_sql_actionplan = mysqli_query($conn, $sql) or die($conn->error . $sql . 'b');
        if ($insert_sql_actionplan) {
            console_log('Success devplanmt_a3_actionplan_tbl');
        };
    endif;

    if ($check_dp_str_behavioral) :
        console_log("Record already exist in devplanmt_b1_strength_tbl");
    else :
        for ($i = 0; $i < count($strbehavioral); $i++) :
            $sql = "INSERT INTO `devplanmt_b1_strength_tbl`(`user_id`, `rater_id`, `sy`, `school`, `position`,  `strength_cbc_id`, `status`,  `date_created`) VALUES ($user,$rater,$sy,$school,'" . $position . "',$strbehavioral[$i],'Active','" . dateNow() . "')";
            $insert_sql_str_behavioral = mysqli_query($conn, $sql) or die($conn->error . $sql . 'a');
        endfor;
        if ($insert_sql_str_behavioral) {
            console_log('Success devplanmt_b1_strength_tbl');
        };
    endif;

    if ($check_dp_devneeed_behavioral) :
        console_log("Record already exist in devplanmt_b2_devneeds_tbl");
    else :
        for ($i = 0; $i < count($devneedbehavioral); $i++) :
            $sql = "INSERT INTO `devplanmt_b2_devneeds_tbl`(`user_id`, `rater_id`, `sy`, `school`, `position`,  `devneed_cbc_id`, `status`,  `date_created`) VALUES ($user,$rater,$sy,$school,'" . $position . "',$devneedbehavioral[$i],'Active','" . dateNow() . "')";
            $insert_sql_devneed_behavioral = mysqli_query($conn, $sql) or die($conn->error . $sql . 'a');
        endfor;
        if ($insert_sql_devneed_behavioral) {
            console_log('Success devplanmt_b2_devneeds_tbl');
        };
    endif;

    if ($check_dp_actionplan2) :
        console_log("Record already exist in devplanmt_b3_actionplan_tbl");
    else :
        $sql = "INSERT INTO `devplanmt_b3_actionplan_tbl`( `user_id`, `rater_id`, `sy`, `school`, `position`, `b_learning_objectives`, `b_intervention`, `b_timeline`, `b_resources_needed`, `status`, `date_created`) VALUES ($user,$rater,$sy,$school,'$position','$b_learn_obj','$b_intervention','$b_timeline','$b_resource_needed','Active','" . dateNow() . "')";
