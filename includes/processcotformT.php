...
if (isset($_POST['save'])) {
    ...
    if ($resultcount > 0) {
        header('location:../setcotform.php?notif=recordexist');
        exit();
    }


    for ($count = 0; $count < count($indicator_id); $count++) {
        $qry = mysqli_query($conn, 'INSERT INTO `cot_t_rating_a_tbl`(`rater_id1`, `rater_id2`, `rater_id3`, `date`, `user_id`, `obs_period`, `indicator_id`, `rating`, `sy`, `school_id`,`status`) VALUES (' . $rater_id . ',' . $rater_id2 . ', ' . $rater_id3 . '," ' . $date . '", ' . $user_id . ', ' . $obs_period . ', ' . $indicator_id[$count] . ', ' . $tcotrating[$count] . ', ' . $sy_id . ', ' . $school_id . ',"' . $status . '")') or die($conn->error . $qry);
    }
    ...
    header('location:../setcotform.php?notif=success');
...
if(isset($_POST['edit'])):
    ...
    $query2 = mysqli_query($conn, 'UPDATE `cot_t_rating_b_tbl` SET `comment` = "'.$comment.'" WHERE `user_id` = '.$user_id.' AND obs_period = '.$obs_period.' AND `school_id` = '.$school_id.' AND `sy` = '.$sy_id.'') or die($conn->error .$query2);

    header("location:../displaytcotprogress.php?notif=success&user_id=$user_id&obs=$obs_period");
    // rpmsdb::generateCOTindicatorAVG($conn, $sy);
    exit();
