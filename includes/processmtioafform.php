...
if (isset($_POST['save'])) {
    ...
    $checkCOTqry = "SELECT * FROM a_mtioafrating_tbl WHERE sy = $sy_id AND obs_period = $obs_period AND school_id=$school_id AND `user_id` = $user_id";
    $cotquery = mysqli_query($conn, $checkCOTqry) or die($conn->error);
    $resultcount = mysqli_num_rows($cotquery);

    if ($resultcount > 0) {
        header('location:../mtioafform.php?notif=recordexist');
        exit();
    }
    ...
    header('location:../mtioafform.php?notif=success');
    exit();
}
