...
function GenderChart() {
    let data = google.visualization.arrayToDataTable([
        ...
        <?php
        $qry = $conn->query("SELECT gender_tbl.gender_name, COUNT(esat1_demographicsmt_tbl.user_id) total FROM esat1_demographicsmt_tbl INNER JOIN gender_tbl on esat1_demographicsmt_tbl.gender = gender_tbl.gender_id WHERE sy = '$sy' GROUP BY gender_tbl.gender_name") or die ($conn->error);
...
function SelfAssessmentChart() {
    let data = google.visualization.arrayToDataTable([
       ['Objective','Low','Moderate','High','Very High'],
        ...
        FROM esat2_objectivesmt_tbl WHERE sy = '$sy' GROUP BY mtobj_id") or die ($conn->error.$qry);
...
function GenderChart() {
    let data = google.visualization.arrayToDataTable([
        ['Gender', 'No. Master of Teacher'],
        <?php
        $qry = $conn->query("SELECT gender_tbl.gender_name, COUNT(esat1_demographicsmt_tbl.user_id) total FROM esat1_demographicsmt_tbl INNER JOIN gender_tbl on esat1_demographicsmt_tbl.gender = gender_tbl.gender_id WHERE sy = '$sy_id' AND school = '$school' GROUP BY gender_tbl.gender_name") or die ($conn->error);
            while ($GenderChart = $qry->fetch_assoc()):
                echo "['".$GenderChart['gender_name']."', ".$GenderChart['total']."],";
            endwhile;?>
]); 
...
function SelfAssessmentChart() {
    let data = google.visualization.arrayToDataTable([
       ['Objective','Low','Moderate','High','Very High'],
        ...
        CASE WHEN priodev = 3 THEN COUNT(user_id) END  AS priohigh,
        CASE WHEN priodev = 4 THEN COUNT(user_id) END  AS prioveryhigh
        FROM esat2_objectivesmt_tbl WHERE sy = '$sy_id' and school = '$school' GROUP BY mtobj_id") or die ($conn->error.$qry);
...
function CoreBehavioralChart() {
    let data = google.visualization.arrayToDataTable([
       ['Core Behavioral','Scale 1','Scale 2','Scale 3','Scale 4','Scale 5'],
        ...
        INNER JOIN core_behavioral_tbl b on a.cbc_id=b.cbc_id
        WHERE sy = '$sy_id' AND school = '$school'
        GROUP BY b.cbc_name") or die ($conn->error.$qry);
